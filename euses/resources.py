...
class Power_Plants():
    def __init__(self, nuts_2):
        ...
        df = pd.read_csv('data/resource/gppd/global_power_plant_database.csv')
        ...
        for nuts_2_geo in nuts_2.ds['geometry']:
            ...
            df1 = pd.DataFrame(gppd.loc[gppd.within(nuts_2_geo.item())].groupby('fuel1').sum().capacity_mw.rename(nuts_2_id)).T
            df = df.append(df1)
        df = df.fillna(0)
